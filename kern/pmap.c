...
mem_init(void)
{
    ...
    size_t i;
    struct PageInfo* newpp = page_alloc(ALLOC_ZERO);
    pte_t* pte;

    for(i = 0; i < n;i += PGSIZE){
        kern_pgdir[PDX(UPAGES + i)] = page2pa(newpp) | PTE_W | PTE_P;
        pte = &((pte_t*)page2kva(newpp))[PTX(UPAGES + i)];
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    for(i = 0;i < size ;i += PGSIZE,pa += PGSIZE){
        vas = va + i;
        pde = &pgdir[PDX(vas)];
        
        pte = pgdir_walk(pgdir,(void*)vas,1);
        *pde = (*pde & ~0x0fff) | (perm | PTE_P);
        if(!pte){
            panic("boot_map_region couldn't allocate page!\n");
        }
        
        *pte = 0;
        *pte = (*pte & ~0x0fff) | (perm | PTE_P);
        *pte |= pa;
    }
}
